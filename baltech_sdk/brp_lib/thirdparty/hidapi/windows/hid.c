...
static HANDLE open_device(const wchar_t *path, BOOL open_rw, BOOL enumerate)
{
	HANDLE handle;
	DWORD desired_access = (open_rw)? (GENERIC_WRITE | GENERIC_READ): 0;
	
	// workaround for allowing enumeration even on opened devices (see SDK-84)
...
struct hid_device_info HID_API_EXPORT * HID_API_CALL hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	for (wchar_t* device_interface = device_interface_list; *device_interface; device_interface += wcslen(device_interface) + 1) {
		...
		device_handle = open_device(device_interface, FALSE, TRUE);
...
HID_API_EXPORT hid_device * HID_API_CALL hid_open_path(const char *path)
{
	...
	device_handle = open_device(interface_path, TRUE, FALSE);
	...
	if (device_handle == INVALID_HANDLE_VALUE) {
		...
		device_handle = open_device(interface_path, FALSE, FALSE);
